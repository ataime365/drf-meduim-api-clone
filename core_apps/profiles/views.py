...
# TODO: change DEFAULT_FROM_EMAIL in production
...
class ProfileDetailAPIView(generics.RetrieveAPIView):
    ...
    permission_classes = [IsAuthenticated,] # A user would need to be authenticated before getting his or her profile
    serializer_class = ProfileSerializer
    renderer_classes = [ProfileJSONRenderer, ]

    def get_queryset(self):
        queryset = Profile.objects.select_related("user") #joining the profile and user table
        return queryset
    
    def get_object(self):
        user = self.request.user #This line gets the user who made the request
        profile = self.get_queryset().get(user=user) #Getting the profile that matches the user
        return profile
...
class UpdateProfileAPIView(generics.RetrieveUpdateAPIView):
    ...
    parser_classes = [MultiPartParser,] #To parse form data #file uploads to profile photo
    renderer_classes = [ProfileJSONRenderer, ]

    def get_object(self): #same as line 41 function
...
class FollowerListView(APIView):
    """To get all followers of the currently logged in user which is me"""
    permission_classes = [IsAuthenticated]
