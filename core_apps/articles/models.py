...
class Clap(TimeStampedUUIDModel):
    """Intermeduate table between article and user"""
...
class Article(TimeStampedUUIDModel):
    ...
    def estimated_reading_time(self):
        return ArticleReadTimeEngine.estimate_reading_time(self) # self = Article

    def view_count(self):
        return self.article_views.count()

    def average_rating(self):
        ratings = self.ratings.all() #ratings is a reverse foreignkey relationship to Articles
...
class ArticleView(TimeStampedUUIDModel):
    """The ArticleView can be many for an article or a user. The Foregnkey falls on the Many Table
    Helps us to track what article was viewed by who and the viewers ip"""
